...
vim.g.loaded_markdown_inline = 1
...
require("kanagawa").setup({
    ...
    -- this part matters for markdown
    colors = {
        theme = {
            all = {
                ui = {
...
vim.api.nvim_set_hl(0, "@markup.bold.markdown", { bold = true })
...
-- lspconfig
-- Neovim 0.11+ LSP configuration (NEW API)

-- optional: shared capabilities (important for cmp)
...
-- leader ll for compiling, lv for viewing pdf, lk for stopping compiler, lc for cleaning aux files

vim.keymap.set("n", "<leader>pv", function()
    vim.fn.jobstart({ "zathura", vim.fn.expand("%:p") }, { detach = true })
end, { desc = "Preview PDF in Zathura" })
